        ...
        <div class="bottom">
          ...
          <div class="operators">
            <div
              ...
              @click="changeMode"
            >
              <i :class='this.mode === 0? "icon-sequence":this.mode===1? "icon-loop": "icon-random"'></i>
            </div>
            <div
              class="
                icon
...
export default {
  ...
  methods: {
    ...
    middleTouchMove(e) {
      ...
      const transformText = `translate3d(${offsetWidth}px,0,0)`;
      this.$refs.lyricList.$el.style[transform] = transformText;
    ...
    leave(el, done) {
      ...
      const { x, y, scale } = this._getPosAndScale();
      const transformText = `translate3d(${x}px,${y}px,0) scale(${scale})`;
      this.$refs.cdWrapper.style[transform] = transformText;
